...
def controlled_go(pos , ori, width):
    global rob, pybullet, control_counter, reached
    if control_counter == 0:
        rob.go(pos, ori, width)
    elif control_counter%10==0:
        reached = rob.check_joint_positions
    control_counter+=1

gripperJoints = rob.get_id_by_name(gripperNames)
gripperControlID = rob.get_control_id_by_name(rob.gripperNames)
robot_func = robot_functions.robot_functions(robot)


initial_joint_positions = [-pi/2,-pi/2,-pi/2,-pi/2,pi/2,pi/2] 
#1 = axis1, 2= axis2, 3= axis3, 4=axis4, 5= axi5, 6= axis6
robot_func.starting_robot_joints(initial_joint_positions)
# rob.go(objStartPos , ori=ori, width=0.13)

# Set initial joint positions manually
final_joint_positions = [-pi/2,-pi/2,-pi/2,-pi/2,pi/2,1.0]   # Example joint positions
end_effector_link_index = sensorID1[0]
...
position, orientation = pybullet.getLinkState(robot, end_effector_link_index)[:2]
print('eef pos=',position)
print('eef ori=',orientation)
robot_func.starting_robot_eef(position, orientation)
    ...
    try:
        for _ in range(1000000):
            ...
            if t==0:        
                ...
                # controlled_go(pos , ori=ori, width=0.13) 
