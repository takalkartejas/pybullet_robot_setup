...
parser.add_argument('-data_path', nargs='?', default='data/grasp', help='Data Path.')
...
force_range_list = {
    "TomatoSoupCan": np.linspace(5, 10, 6).tolist(),
    "RubiksCube": np.linspace(5, 10, 6).tolist(),
    "MustardBottle": np.linspace(5, 10, 6).tolist(),
}


height_range_list = {
    "TomatoSoupCan": utils.heightReal2Sim(np.array([110, 103, 96, 88, 81, 74, 67])),
    "MustardBottle": utils.heightReal2Sim(np.array([125, 142, 154, 190])),
    "RubiksCube": utils.heightReal2Sim(np.array([58, 57, 56, 55])),
}

if args.dxy:
    dx_range_list = defaultdict(lambda: np.linspace(-0.005, 0.005, 5).tolist())
else:
    dx_range_list = defaultdict(lambda: [0.0])
