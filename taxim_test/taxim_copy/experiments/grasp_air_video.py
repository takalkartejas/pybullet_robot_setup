...
force_range_list = {
    "044_flat_screwdriver": np.linspace(5, 10, 6).tolist(),
    "037_scissors": np.linspace(5, 10, 6).tolist(),
    "033_spatula":np.linspace(5, 10, 6).tolist(),
}

dx_range_list = defaultdict(lambda: np.linspace(-0.015, 0.02, 10).tolist())
dx_range_list['044_flat_screwdriver'] = np.array([0.0, 0.01, 0.02, 0.05, 0.08, 0.10]) + 0.04
dx_range_list['033_spatula'] = np.array([0.05, -0.05, 0.006, -0.018]) - 0.02 #
dx_range_list['037_scissors'] = np.array([-0.03, -0.06, 0.0, 0.03, 0.06]) + 0.03
...
if __name__ == "__main__":
    ...
    while True:
        if t == 0:
            ...
            pb.changeDynamics(objID, -1, mass=0)
            height_grasp = objStartPos[2] + utils.ee_gap + 0.02 #0.020 #0.022 #0.015
            rot = 0.0
            bias_lth = 0.012
            ...
            data_dict["height"], data_dict["gripForce"], data_dict["rot"] = utils.heightSim2Real(
                pos[-1]), gripForce, rot
            objPos0, objOri0 = utils.get_object_pose(pb, objID)
            ...
            if args.gui:
                gelsight.updateGUI(tactileColor_tmp, depth)
            objPos, objOri = utils.get_object_pose(pb, objID)
            ...
            if log.id < 1:
                ...
                save_name = f'F{gripForce}_H{height_real}_dx{dx_real}_dy{dy}_L{label}'
                sub_save_dir = os.path.join(save_dir, save_name)
                os.makedirs(sub_save_dir, exist_ok=True)

                for i, seq_img in enumerate(seq_tactile):
                    cv2.imwrite(os.path.join(sub_save_dir, str(i)+'.jpg'), seq_img)
        ...
        ### seq data
        if t >= 100 and t % 5 == 0 and cur_sample < total_sample:
            tactileColor_tmp, depth = gelsight.render()
            seq_tactile.append(tactileColor_tmp[0].copy())
            cur_sample += 1
