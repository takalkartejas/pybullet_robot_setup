...
def getObjInfo(objName):
    if objName == "Cube":
        urdf_path = os.path.join("app/Taxim/experiments/setup/objects", objName, "cube_small.urdf")
        force_range, deformation = FT_data_dict["RubiksCube"]
        return urdf_path, 1., 0.05, force_range, deformation
    assert objName in obj_list
    urdf_path = os.path.join("/app/Taxim/experiments/setup2/objects", objName, "model.urdf")
