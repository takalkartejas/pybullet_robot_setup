...
class Sensor:
    def __init__(
            ...
            height=160,
            background=None,
            config_path=get_gelsight_config_path(),
            visualize_gui=True,
            show_depth=True,
            zrange=0.004,
            cid=0,
    ...
    #tt-obj_id = robot, link_id = finger on which sensor is mounted
    def add_camera(self, obj_id, link_ids):
        ...
        """
        #tt-Checks if link_ids is a sequence. If it's not a sequence, it converts it to a single-element list containing link_ids.
        if not isinstance(link_ids, collections.abc.Sequence):
            link_ids = [link_ids]
        
        for link_id in link_ids:
            #nb cam is initialized as 0 and then incremented
            cam_name = "cam" + str(self.nb_cam)
            # tt- self.camera is an empty dictionary, link is just a data class, cid = physics client id
            # tt- for grasp_data_demo self.camera = {'cam0': Link(obj_id=1, link_id=10, cid=0)}
    ...
    def get_force(self, cam_name):
        ...
        # print("shear force", self.shear_forces[cam_name])
